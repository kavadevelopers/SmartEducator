...
@section('content')
    ...
    <div class="page-body">
        <form method="post" action="<?= App\Http\Controllers\admin\BaseController::aUrl('/courses/edit') ?>" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="card">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="category" required>
                                ...
                                <option value="1" <?= $item->category == '1'?'selected':'' ?>>Graduation Course</option>
                                <option value="2" <?= $item->category == '2'?'selected':'' ?>>Post Graduation Course</option>
                            </select>
                        </div>
                    </div> 
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea name="about" type="text" class="form-control" placeholder="About the program" required><?= old("duration",$item->about) ?></textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Specialization <span class="-req">*</span></label>
                            <textarea name="specialization" type="text" class="form-control" value="<?= old("specialization") ?>" placeholder="Specialization" required><?= old("duration",$item->specialization) ?></textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Eligibility Criteria  <span class="-req">*</span></label>
                            <textarea name="eligibility" type="text" class="form-control" value="<?= old("eligibility") ?>" placeholder="Eligibility Criteria " required><?= old("duration",$item->eligibility) ?></textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Job role <span class="-req">*</span></label>
                            <textarea name="job_role" type="text" class="form-control" value="<?= old("job_role") ?>" placeholder="Job role" required><?= old("duration",$item->job_role) ?></textarea>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label>Admission Process <span class="-req">*</span></label>
                            <textarea name="process" type="text" class="form-control" value="<?= old("process") ?>" placeholder="Admission Process" required><?= old("duration",$item->process) ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
